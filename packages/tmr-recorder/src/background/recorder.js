...
export default class BackgroundRecorder {
  ...
  async attachToTab(tabId) {
    ...
    if (!attached) {
      throw new Error('Recorder content script not available in target tab.')
    }
  }
  ...
  async attachWithTab(sessionId, tab) {
    if (this.attached || this.isAttaching) {
      return
    }
    try {
      ...
      this.initSession(tab, sessionId)
      this.windowSession.openedTabCount[sessionId] = 1
