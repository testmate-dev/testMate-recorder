...
const assignUiIds = steps => {
  ...
  return steps.map(step => {
    const key = stepKey(step)
    const existing = reusableIds.get(key) || []
    const _uiId = existing.length > 0 ? existing.shift() : `step-${stepIdCounter++}`
    reusableIds.set(key, existing)
    return {
      ...step,
      _uiId,
...
const captureStepRects = () => {
  ...
  rows.forEach(row => {
    rects.set(row.dataset.stepId, row.getBoundingClientRect())
  })
  return rects
}

const animateStepReorder = previousRects => {
  const rows = stepsEl.querySelectorAll('.step')
  rows.forEach(row => {
    const before = previousRects.get(row.dataset.stepId)
    if (!before) return
    ...
    requestAnimationFrame(() => {
      row.style.transition = 'transform 240ms cubic-bezier(0.22, 0.61, 0.36, 1)'
      row.style.transform = 'translate3d(0, 0, 0)'
    })
  })
}

const renderSteps = options => {
  const opts = options || {}
  const previousRects = opts.animate ? captureStepRects() : null
  ...
  uiSteps.forEach((step, index) => {
    ...
    row.className = 'step'
    row.dataset.stepId = step._uiId
    if (draggingStepId === step._uiId) {
  ...
  if (previousRects) {
    animateStepReorder(previousRects)
  }
}

const indexByStepId = stepId => uiSteps.findIndex(step => step._uiId === stepId)

const moveStep = (fromIndex, toIndex) => {
  if (
    fromIndex < 0 ||
    toIndex < 0 ||
    fromIndex >= uiSteps.length ||
    toIndex >= uiSteps.length ||
    fromIndex === toIndex
  ) {
    return false
  }
  const [moved] = uiSteps.splice(fromIndex, 1)
  uiSteps.splice(toIndex, 0, moved)
  return true
}
...
const syncDraggingClass = () => {
  const rows = stepsEl.querySelectorAll('.step')
  rows.forEach(row => {
    if (row.dataset.stepId === draggingStepId) {
      row.classList.add('dragging')
    } else {
...
const processDragMove = clientY => {
  if (!isDragging || !draggingStepId) return
  const fromIndex = indexByStepId(draggingStepId)
  const toIndex = getDropIndex(clientY)
  console.debug('[tmr-ui][drag] over', {
    stepId: draggingStepId,
    fromIndex,
    toIndex,
  })
  if (moveStep(fromIndex, toIndex)) {
    console.debug('[tmr-ui][drag] moved', {
      stepId: draggingStepId,
      fromIndex,
      toIndex,
      orderReadable: debugStepOrder(),
    })
    renderSteps({ animate: true })
  }
}
...
async function onDragMouseUp() {
  ...
  console.debug('[tmr-ui][drag] drop', {
    stepId: draggingStepId,
    orderReadable: debugStepOrder(),
  })
  removeDragListeners()
...
const finishDrag = async () => {
  ...
  const hasChanged = currentOrder !== dragStartOrder
  console.debug('[tmr-ui][drag] finish', {
    draggingStepId,
    hasChanged,
    before: dragStartOrder,
    after: currentOrder,
    afterReadable: debugStepOrder(),
  })
  isDragging = false
  draggingStepId = null
  dragStartOrder = ''
...
stepsEl.addEventListener('mousedown', event => {
  if (event.button !== 0) return
  const handle = event.target.closest('.step-handle')
  if (!handle) return
  event.preventDefault()
  const stepId = handle.dataset.stepId
  const stepIndex = indexByStepId(stepId)
  if (stepIndex < 0) return
  isDragging = true
  draggingStepId = stepId
  dragStartOrder = uiSteps.map(step => step._uiId).join('|')
  console.debug('[tmr-ui][drag] start', {
    stepId,
...
window.addEventListener('blur', async () => {
  if (!isDragging) return
  console.debug('[tmr-ui][drag] cancel(blur)', {
    stepId: draggingStepId,
    orderReadable: debugStepOrder(),
  })
  removeDragListeners()
...
const callBg = async (method, ...args) => {
  try {
    ...
    if (method !== 'status' && method !== 'steps') {
      console.debug('[tmr-ui] calling(runtime.sendMessage)', method, message)
    }
    ...
    console.warn('[tmr-ui] background unavailable', {
      method,
      apiRef,
      response,
    })
